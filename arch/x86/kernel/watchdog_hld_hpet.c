...
#ifdef HACK_TOPOLOGY
#ifdef topology_logical_die_id
#undef topology_logical_die_id
static u16 topology_logical_die_id(int cpu)
{
	switch (cpu) {
	case 0:
	case 2:
...
#ifdef topology_die_cpumask
#undef topology_die_cpumask

DEFINE_PER_CPU_READ_MOSTLY(cpumask_var_t, my_cpu_die_map);
EXPORT_PER_CPU_SYMBOL(my_cpu_die_map);

static void hack_topology(void)
{	int i, j;

	for_each_possible_cpu(i) {
		if(!zalloc_cpumask_var(&per_cpu(my_cpu_die_map, i), GFP_KERNEL)) {
			ricardo_printk("topology hack failed!\n");
			return;
		}
		for_each_possible_cpu(j) {
			if (!((j + i) % 2))
				cpumask_set_cpu(j, per_cpu(my_cpu_die_map, i));
		}
		ricardo_printk("HACKED TOPOLOGY. Die mask CPU%d: %*pbl\n", i, cpumask_pr_args(per_cpu(my_cpu_die_map, i)));
	}
	topology_hack_enabled = true;
}

#define topology_die_cpumask(cpu)		(per_cpu(my_cpu_die_map, cpu))
#endif
#else
static void hack_topology(void) { }
#endif
...
static int cpumasks_show(struct seq_file *m, void *data)
{
	...
	for_each_possible_cpu(i) {
		...
		ricardo_printk("topology %d[id:%d] %*pb\n", i, topology_logical_die_id(i), cpumask_pr_args(per_cpu(my_cpu_die_map, i)));
	}
#endif
...
/**
 * get_first_cpu_in_next_die() - Find the first CPU in the next die
 * @start_cpu:	CPU from which we start looking
 * @hdata:	A data structur with the monitored CPUs mask
 *
 * Find the first CPU in the tie next to the die of @start_cpu. If there is
 * only one die/package in the system, return @start_cpu.
 */
static unsigned int get_first_cpu_in_next_die(int start_cpu,
					      struct hpet_hld_data *hdata)
{
	...
	this_cpu_die_id = topology_logical_die_id(start_cpu);
	...
	while (this_cpu_die_id == next_cpu_die_id && safe < 400) {
		...
		next_cpu_die_id = topology_logical_die_id(next_cpu);
 ...
 * CPUs in the updated target_cpumask are in the same die.
 *
 * The target_cpumask start to be updated at the first monitored CPU in the
 * next die from the handling CPU.
 */
...
static void setup_cpu_groups(struct hpet_hld_data *hdata)
{
	...
	while (watchdog_thresh < hdata->nr_groups && safe < 400) {
		...
		hdata->nr_groups = DIV_ROUND_UP(monitored_dies,
						hdata->dies_per_group);
		ricardo_printk("thresh %d monitored_dies %d nr_groups %d dies_per_group %d\n",
			watchdog_thresh, monitored_dies, hdata->nr_groups, hdata->dies_per_group);
