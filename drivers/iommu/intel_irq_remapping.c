...
int watchdog_hld_hpet_set_destid(struct hpet_hld_data *hdata)
{
	struct intel_ir_data *data = (struct intel_ir_data *)hdata->ir_data;

	data->irte_entry.dest_id = IRTE_DEST(apic->calc_dest_apicid(hdata->handling_cpu));
